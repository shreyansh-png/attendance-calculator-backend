...
const uploadTimetable = async (req, res) => {
    try {
        ...
        // Create Subject Map
        // =====================================
        //optimisation : instead of finding each subject one by one we find all subjects in one query
        const subjectMap = {};

        // Get unique subject codes
        const uniqueSubjectCodes = [
            ...new Set(timetableRows.map(row => row.subjectCode))
        ];
        // Fetch all existing subjects in one query
        const existingSubjects = await Subject.find({
            subjectCode: { $in: uniqueSubjectCodes }
        });
        // Store existing subjects in the map
        existingSubjects.forEach(subject => {
            subjectMap[subject.subjectCode] = subject._id;
        });
        // Create only missing subjects
        for (const row of timetableRows) {
            if (subjectMap[row.subjectCode]) {
                continue;
            }
...
const getTodayTimetable = async (req, res) => {
    try {
        ...
        const populateFields = DAYS.map(day => ({
            path: `schedule.${day}.subjectId`
        }));
        ...
        if (today === "sunday") {
            ...
            return res.status(200).json({
                success: true,
                message: "No classes today , Have fun IETIANS 😎",
                classes: []
            });
